...
class EditJobScreen: Screen {
    ...
    override fun Content() {
        ...
        val latestJob = viewModel.latestAddedJob.observeAsState()

        var companyName by remember { mutableStateOf(jobDetail.companyName) }
        var jobPosition by remember { mutableStateOf(jobDetail.jobPosition) }
        var appliedDate by remember { mutableLongStateOf(jobDetail.appliedDate.time) }
        var deadline by remember { mutableLongStateOf(jobDetail.deadline.toInstant().toEpochMilli()) }
        var additionalNote by remember { mutableStateOf(jobDetail.additionalNote) }
